...
로컬 개발 환경에서 원본 저장소의 기본 위치는 다음과 같다.

```text
../worship-conti-maker-node-version
```
...
버전 숫자는 이 헌장에 고정된 영구 정본이 아니다. 각 구현 슬라이스를 시작할 때 원본 문서의 현재 버전과 관련 요구사항을 다시 확인한다.
